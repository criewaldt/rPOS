	...
	if (obj instanceof Object) {
		for (var i=0;i<childKey.length;i++) {
			...
			if (obj[parentKey][childKey[i]].price){
				createItem(jsonID,id,elementProperties,obj[parentKey][childKey[i]].price);
			...
			HTMLgenerator(jsonID,id,elementProperties);
...
HTMLelement(a)
	IN: a - variable argument. it is in the form
				...
				number of properties to be set for the new element
...
function HTMLelement(a) {
	...
	var Element = document.createElement(a);
		...
		if (elementProperties[0].FONT_SIZE) {
			console.log(elementProperties[0].BACKGROUND_COLOR);
