...
var BUTTON_BUILDER;// used to hold the JSON sent from server used to build .... everything
...
function ButtonBuilder(obj,parentKey,id) {
	...
	//they need to be removed because they are not intended to be actual HTML elements
	var CustomStyle = false;
	var itemInfo = false;
	for (var x =0; x<childKey.length;x++) {
		if (childKey[x]  === 'CustomStyle' || childKey[x]  === 'itemInfo'){
...
HTMLelement(a)
	IN: a - variable argument. it is in the form
				...
				for example change background color, text size or inner html. these come in as the property : CustomStyle.
...
function HTMLelement(a) {
	...
	//if 'custom' styles, overwrite the defaults from HTMLgenerator() and createItem() with CustomStyle
...
function addItemToOrder(id,count,price,mod){
	...
	//	ORDER_TOTAL += parseFloat(price);
		...
		var newItem = HTMLelement("ul",{id:itemName,
													value:price,
													name:id});
...
idea now is to itterate the children of orderArea, check if each element has children
of its own (these are the modifiers) and add to the ORDER_DATA json
//ORDER_DATA = {'ORDER_DATA' : {}};

format: {'ORDER_DATA' : {'order_1':{'item'{},'mods':{}} } }
*/
function create_orderData_json(){
	var orderArea = $('#orderArea');
	//console.log(orderArea.children().length);
	//console.log(orderArea[0].childNodes);
   for (var i=1; i<=orderArea.children().length; i++){
   	console.log(orderArea[0].childNodes[i].attributes.name);
		var locInButtonBuilder = orderArea[0].childNodes[i].attributes.name.value;
		console.log(locInButtonBuilder);
	}
};
...
$(document).ready(function(){
	...
	document.getElementById('sendOrder').onclick = function (){
		...
		/*    UN COMMENT BELOW, COMMENTED DURING TESTING ONLY  */
		//$('#orderArea').empty();
	};
	...
	mySocket.on('message', function(msg) {
			...
			if (Object.keys(JSONdata)[0] === 'buttonBuilder'){
				...
				BUTTON_BUILDER = JSONdata;//set global
